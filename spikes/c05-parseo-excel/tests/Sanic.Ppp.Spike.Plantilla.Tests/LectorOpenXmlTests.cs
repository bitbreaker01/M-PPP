...
{
    public class LectorOpenXmlTests
    {
        ...
        [Fact]
        public void LP05_no_pierde_una_fila_util_que_viene_despues_de_una_fuera_de_rango_en_el_archivo()
        {
            // Hallazgo A: <row r="500"> ANTES que <row r="6"> (la util) en el archivo. Un corte
            // por "ya vi una fila fuera de rango, asumo que ya pase todo" se dispara con la 500
            // y la fila 6 (la que importa) nunca se lee.
            var config = ConfigSimple(filaEncabezado: 5, primeraFila: 6, cantidadFilas: 1);
            byte[] xlsx = new ConstructorXlsxDePrueba("Datos")
                .ConTexto("C5", "Campo Uno").ConTexto("D5", "Campo Dos")
                .ConTexto("C500", "basura").ConTexto("D500", "basura")
                .ConTexto("C6", "fila-util").ConTexto("D6", "d1")
                .ConOrdenDeFilasExplicito(5, 500, 6)
                .Bytes();
            ...
            Assert.True(resultado.EsValido, string.Join(" | ", resultado.Errores));
            Assert.Single(resultado.Filas);
            Assert.Equal("fila-util", resultado.Filas[0].Valores["C"]);
        }
        ...
        [Fact]
        public void LP04_rechaza_archivo_que_supera_el_tope_chico_de_filas_recorridas()
        {
            // Hallazgo C (filas recorridas). Tope de 50, archivo con 60 filas (todas fuera de
            // rango salvo el encabezado): se recorren, no se materializan, pero igual cuentan.
            var config = ConfigSimple(filaEncabezado: 5, primeraFila: 6, cantidadFilas: 1);
            var constructor = new ConstructorXlsxDePrueba("Datos")
                .ConTexto("C5", "Campo Uno").ConTexto("D5", "Campo Dos");
            for (int fila = 100; fila < 160; fila++)
                constructor.ConTexto("C" + fila, "basura");
            byte[] xlsx = constructor.Bytes();

            var lector = new LectorOpenXml(new LimitesLectura { MaximoFilasRecorridas = 50 });
            var resultado = lector.Leer(xlsx, config);

            Assert.False(resultado.EsValido);
            Assert.Contains(resultado.Errores, e => e.Contains("filas recorridas"));
        }

        [Fact]
        public void LP08_rechaza_una_fila_que_supera_el_tope_chico_de_celdas_por_fila()
        {
            var config = ConfigSimple(filaEncabezado: 5, primeraFila: 6, cantidadFilas: 1);
            byte[] xlsx = new ConstructorXlsxDePrueba("Datos")
                .ConTexto("C5", "Campo Uno").ConTexto("D5", "Campo Dos")
                .ConTexto("A6", "1").ConTexto("B6", "2").ConTexto("C6", "3")
                .ConTexto("D6", "4").ConTexto("E6", "5")
                .Bytes();

            var lector = new LectorOpenXml(new LimitesLectura { MaximoCeldasPorFila = 3 });
            var resultado = lector.Leer(xlsx, config);

            Assert.False(resultado.EsValido);
            Assert.Contains(resultado.Errores, e => e.Contains("celdas por fila"));
        }
        ...
        [Fact]
        public void Hallazgo_F_filas_recorridas_y_materializadas_reflejan_lo_que_de_verdad_paso()
        {
            // Reemplaza el test viejo que "pasaba igual sin ninguna proteccion" (hallazgo F): en
            // vez de solo comprobar que la fila util se lee, se afirma sobre la metrica real que
            // demuestra que LP-04/05 funcionan.
            var config = ConfigSimple(filaEncabezado: 5, primeraFila: 6, cantidadFilas: 1);
            var constructor = new ConstructorXlsxDePrueba("Datos")
                .ConTexto("C5", "Campo Uno").ConTexto("D5", "Campo Dos")
                .ConTexto("C6", "fila-util").ConTexto("D6", "d1");
            const int filasBasura = 40;
            for (int fila = 7; fila < 7 + filasBasura; fila++)
                constructor.ConTexto("C" + fila, "basura");
            byte[] xlsx = constructor.Bytes();
            ...
            Assert.True(resultado.EsValido, string.Join(" | ", resultado.Errores));
            Assert.Single(resultado.Filas);
            // encabezado (1) + fila util (1) + basura (40) = 42 filas <row> en el archivo.
            Assert.Equal(2 + filasBasura, resultado.FilasRecorridas);
            // solo se materializan (LoadCurrentElement) las que caen en rango: encabezado + util.
            Assert.Equal(2, resultado.FilasMaterializadas);
        }
    }
}
