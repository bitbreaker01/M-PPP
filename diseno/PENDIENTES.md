...
| D-6 | Relaciones 4.10 y 4.11 (BP-PP-188) | **Nombre de dos relaciones entre el mismo par de tablas.** Fila tiene dos lookups a `systemuser` (digitada por, aprobada por) y la regla `<padre>_<hijo>` les da el mismo nombre. Propuesta: sufijo con el nombre del lookup sin prefijo → `sanic_mppp_systemuser_fila_digitadapor` y `sanic_mppp_systemuser_fila_aprobadapor`. **El nombre de una relación es irreversible**: esas dos no se construyen sin el visto bueno del aprobador. Si lo aprueba, la regla se agrega a BP-PP-188. |
| D-7 | `02` §3.3 y §3.4 | **¿El lookup `sanic_solicitudid` de Resultado de regla y de Bitácora es opcional a propósito?** En Fila lleva `S`; en estas dos, no. Son relaciones parentales: un registro sin solicitud no tendría sentido. Se construyen **como dice el diccionario (opcionales)**; el nivel de requerida se cambia después sin costo. No frena nada. |
