...
| D-11 | `04` §2 (D-25) y `03` §4 | **ABIERTA, con propuesta ya implementada y probada. ¿Qué pasa con una identidad que trae más de un rol cuando uno de ellos es el RPA?** D-25 solo dice que Ejecutivo y Supervisor son excluyentes por usuario. El diseño no dice nada de `Ejecutivo + RPA` ni de `Supervisor + RPA`. **Propuesta del arquitecto: falla cerrado, igual que D-25**: cualquier combinación de roles no puede hacer ninguna transición. El RPA es una identidad de aplicación aparte (`04` §3); si además trae un rol humano, es una mala configuración y lo seguro es que no mueva nada hasta que alguien la corrija. Lo contrario (dejarla actuar con el rol que alcance) permitiría, por ejemplo, que el usuario del bot con rol Supervisor apruebe lo que él mismo digitó saltándose `rpa.puedeaprobar`. Está fijado en las pruebas de aceptación de 7.1 Dominio; si el aprobador decide otra cosa, se cambia la prueba y el código. Salió de la revisión de código de 7.1 (2026-09-21). |
