...
| D-4 | `02` §2.6, tabla Regla | **Cómo se llena la columna primaria `sanic_nombre` de Regla.** El estándar (BP-PP-192, `01` §4) dice que una primaria nunca es texto libre: es calculada, autonumérica o una clave de negocio validada. Las demás tablas lo declaran (Cliente: razón social; Plan: calculada; Autorizado: el correo; Parámetro: el código); Regla dice solo "Primaria". Lo detectó el verificador de la tabla (2026-09-20). La tabla ya está construida como texto de 200, requerida, y **ninguna opción cambia su estructura**: (a) **calculada** por plugin, `<código> - <texto corto>`; (b) **excepción declarada**: es una etiqueta para personas que mantiene solo el Administrador técnico, y la identidad de la regla es `sanic_codigo`, que ya es clave alternativa; (c) que la primaria **sea** el código, como en Parámetro (dejaría `sanic_codigo` redundante: no recomendada con la tabla ya creada). Hay que decidirlo antes del plugin 7.10 y del formulario de Regla (inventario 12.3). |
| D-5 | `02` §3.3 y §3.4 | **¿La primaria de Resultado de regla y la de Bitácora son opcionales a propósito?** En las otras ocho tablas `sanic_nombre` lleva `S`; en estas dos, no. Lo detectó el constructor (2026-09-20). Se construyen **como dice el diccionario: opcionales**. Si fue una omisión, se agrega la `S` al diccionario y se ajusta la columna: el nivel de requerida se cambia sin costo. No frena nada. |
