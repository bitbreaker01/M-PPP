...
| D-10 | `01` §7 (BP-PP-197) | **ABIERTA. Siete nombres visibles con tilde que el Web API no deja cambiar.** (a) Las 6 claves alternativas con tilde (`KEY - MPPP - Plan - Código`, `… Autorización - Autorizado y plan`, `… Parámetro - Nombre y versión`, `… Regla - Código`, `… Fila - Solicitud y número de fila`, `… Resultado de regla - Solicitud y código de regla`): `PUT …/Keys` responde 405. (b) El conjunto de opciones local de la columna sí/no `sanic_mppp_tbl_solicitud.sanic_requiererevision`, que quedó como "Requiere revisión": la columna ya se llama "Requiere revision", pero ese nombre interno no la sigue y el Web API no lo cambia (ensayado por cuatro vías). Opciones para los dos: **borrar y recrear** (tablas vacías, nada los usa todavía; ~15 min las claves, inmediato la columna), cambiarlos a mano en el portal, o aceptarlos como excepción documentada. El de (b) no lo ve el usuario final; solo aparece en el XML de la solución. No frena nada. |
