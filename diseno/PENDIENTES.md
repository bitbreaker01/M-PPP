...
| D-12 | `03` §7, LP-04 y LP-05 | **ACLARACIÓN, a confirmar por el aprobador. Qué pasa con lo que viene DESPUÉS del corte de LP-04.** LP-04 dice que el lector deja de leer en la primera fila más allá de la ventana; el "por qué" de LP-05 da a entender que un archivo con la fila 500 escrita antes que la 6 se rechaza. Las dos cosas a la vez son imposibles: si se corta en la 500, la 6 que viene después **no se llega a ver**, y para detectar ese desorden habría que seguir leyendo, que es el costo no acotado que LP-04 eliminó. **Lectura que se implementa (la literal de LP-04):** el orden se exige en todo lo que se llega a recorrer; un salto *en orden* más allá de la ventana corta la lectura y no invalida nada (caso legítimo: pocas filas llenas y una nota al pie); lo que venga después del corte no se mira. **Por qué es seguro:** Excel nunca escribe ese archivo; y quien lo arme a propósito no gana nada que no consiga con solo no mandar esas filas. **La alternativa descartada** (rechazar si la ventana no estaba completa al saltar) rompe archivos legítimos: la encontró el revisor de código como bloqueante. Salió de la revisión de 7.2 Plantilla (2026-09-21): la prueba de aceptación del arquitecto pedía lo imposible y el constructor la cumplió de la única forma que podía. Fijado en `LectorVentanaYOrdenAceptacion.cs`. |
