...
| D-8 | `04` §4 | **ABIERTA. Un perfil de seguridad de columna se asigna a USUARIOS o EQUIPOS, no a roles, y los permisos son del perfil entero, no de cada miembro** (verificado en Learn el 2026-09-21: "Column-level security to control access" y "Column-level security by using code"). La matriz pone roles como "miembros" de un único perfil con permisos distintos por miembro: eso no existe en la plataforma. Hay que decidir: **(a)** cómo llegan las personas al perfil: asignación usuario por usuario como tarea de administración en cada entorno (igual que la asignación de roles; respeta "sin teams"), o un equipo por rol (menos trabajo manual, pero rompe "sin teams" de `04` §0); **(b)** cuántos perfiles: en fase 1 alcanza con **uno de solo lectura** (Ejecutivo, Supervisor); Histórico (fase 3) necesita leer **y crear**, así que ahí nace un **segundo perfil**. El RPA (fase 2) entra al de solo lectura. **Frena**: el playbook del perfil (inventario 6.1). No frena los roles (6.2–6.6). |
| D-9 | `03` y `04` §3 | **ABIERTA. Qué privilegio gobierna quién puede ejecutar cada Custom API.** La matriz dice que el rol de ingesta "tiene" `clasificarcorreo` y `validarsolicitud`, pero en la plataforma eso se expresa con `ExecutePrivilegeName`, que tiene que nombrar un privilegio **que ya exista**: no se puede crear uno a medida (Learn, FAQ de Custom API, verificado 2026-09-21). Hoy `03` no lo define. Opciones: **(a, recomendada)** usar `prvCreatesanic_mppp_tbl_solicitud` para las dos de fase 1: en fase 1 solo el Servicio de ingesta crea Solicitudes, y tiene sentido de negocio (quien puede dar de alta una solicitud puede clasificarla y validarla); en fase 3 Histórico también lo tendría, y se acepta o se revisa ahí. **(b)** dejarlo vacío y que el plugin compruebe quién llama: más código y un control menos declarativo. **(c)** una tabla marcadora por Custom API solo para tener su privilegio: tablas sin datos, más componentes. Para fase 2: el RPA solo tiene lecturas que comparte con el Ejecutivo, así que (a) no le alcanza y habrá que elegir entre (b) y (c). **Frena**: el registro de Custom API (inventario 8) y el rol 6.6; no frena 6.2–6.5. |
