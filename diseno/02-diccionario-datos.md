...
| ID | Decisión | Motivo |
|---|---|---|
| DD-01 | **La Fila guarda solo valores válidos en los campos de lista.** Gestión, Clasificación, Moneda, Tipo de identificación y Banco son **choices** (decisión del aprobador: todas las listas se tratan igual). Si el cliente mandó un valor inválido, la columna queda vacía y el motivo va en `sanic_mensaje` **nombrando el campo, sin citar el valor** (D-41, aprobador 2026-09-21: el mensaje es por fila, con eso el cliente ubica la celda; el marcador `{valor}` queda disponible si el negocio quiere citarlo en la redacción del catálogo). Los campos de texto libre se guardan como llegaron, recortados al largo de la columna. | Decisión del aprobador: un valor inválido no sirve para nada guardado, y la evidencia de lo que mandó el cliente no se pierde porque el Excel original está en la Solicitud. Reemplaza al diseño anterior de texto crudo más columnas normalizadas. |
...
Regla de integridad (plugin PreOperation, BP-PP-051): el cliente del plan debe ser el cliente del autorizado. **Autorización vigente** (definición única; la usan `REMITENTE_RECONOCIDO` y `AUTORIZACION_CORREO_PLAN`, `03` §0 y §1): están **activos** el Autorizado, el Plan, el Cliente y la propia AutorizacionPlan. **La evidencia no cuenta para la vigencia** (D-42, aprobador 2026-09-21): es el respaldo que el administrador de planes tiene que cargar, y la app se lo muestra pendiente, pero al cliente no se lo afecta porque el banco no lo cargó.

\* **Por qué "obligatoria" no es "requerida al crear".** Una columna de archivo de Dataverse solo acepta el archivo después de que el registro existe (verificado en Learn, 2026-09-20), así que no se puede exigir en el alta. **D-42 (aprobador, 2026-09-21): la falta de evidencia NO invalida la autorización.** La obligatoriedad es administrativa: la vista "Autorizaciones sin evidencia" y la marca en cada subgrilla (`05`) le muestran al Administrador de planes qué le falta cargar. Antes (E-17, 2026-09-20) una autorización sin evidencia no valía; se cambió porque no se afecta al cliente por algo que le faltó hacer al banco.
...
| `sanic_mensaje` | M(4000) | | | Los motivos de las reglas de registro que **fallaron**, todos y no solo el primero; nombran el campo y no citan el valor (D-41). No lleva historial regla por regla: eso es del sobre (3.3). Después se agrega el motivo de la anulación o de la devolución. |
| `sanic_fechavalidada` | F | | | |
