# Playbook: choice-global · sanic_mppp_ch_gestion
...
De dónde sale cada dato: todos de `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. El componente está definido en `diseno/02-diccionario-datos.md` §1 y figura en `diseno/06-inventario-componentes.md` §2, renglón 2.2.
...
```
python3 herramientas/construir/choice_global.py playbooks/choice-global/sanic_mppp_ch_gestion.md
```

## 5. Verificación

```
python3 herramientas/construir/choice_global.py playbooks/choice-global/sanic_mppp_ch_gestion.md --solo-verificar
```
...
`DELETE GlobalOptionSetDefinitions(Name='sanic_mppp_ch_gestion')`, posible solo mientras ninguna columna lo use. **Irreversible desde que tenga uso**: el nombre lógico `sanic_mppp_ch_gestion` y los valores de sus 3 opciones una vez que haya datos con ellos; nunca se renumeran ni se reutilizan.

## 8. Fuera de alcance

La columna `sanic_gestion` de Fila: nace con el playbook de esa tabla (inventario 3.8).
