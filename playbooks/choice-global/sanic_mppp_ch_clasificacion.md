# Playbook: choice-global · sanic_mppp_ch_clasificacion
...
De dónde sale cada dato: todos de `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. El componente está definido en `diseno/02-diccionario-datos.md` §1 y figura en `diseno/06-inventario-componentes.md` §2, renglón 2.3.
...
```
python3 herramientas/construir/choice_global.py playbooks/choice-global/sanic_mppp_ch_clasificacion.md
```

## 5. Verificación

```
python3 herramientas/construir/choice_global.py playbooks/choice-global/sanic_mppp_ch_clasificacion.md --solo-verificar
```
...
|---|---|---|
| 1 | `GET GlobalOptionSetDefinitions(Name='sanic_mppp_ch_clasificacion')` | 200 · `IsGlobal = true` · `OptionSetType = Picklist` · `IsManaged = false` |
| 2 | Display name y descripción en 1033 | los de la sección 2 |
| 3 | Opciones, en orden, con valor, etiqueta y descripción | `159460001 = BAC`, `159460002 = ACH`, `159460003 = CK`; ninguna más; ninguna etiqueta en otro idioma |
...
`DELETE GlobalOptionSetDefinitions(Name='sanic_mppp_ch_clasificacion')`, posible solo mientras ninguna columna lo use. **Irreversible desde que tenga uso**: el nombre lógico `sanic_mppp_ch_clasificacion` y los valores de sus 3 opciones una vez que haya datos con ellos; nunca se renumeran ni se reutilizan.

## 8. Fuera de alcance

La columna `sanic_clasificacion` de Fila: nace con el playbook de esa tabla (inventario 3.8).
