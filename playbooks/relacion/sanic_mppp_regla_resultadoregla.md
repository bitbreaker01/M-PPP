# Playbook: relación · sanic_mppp_regla_resultadoregla
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La relación está definida en `diseno/02-diccionario-datos.md` §3.3 y §5 y figura en `diseno/06-inventario-componentes.md` §4, renglón 4.9.
...
{
  "tipo": "relacion",
  "nombre": "sanic_mppp_regla_resultadoregla",
  "tabla_padre": "sanic_mppp_tbl_regla",
  "tabla_hija": "sanic_mppp_tbl_resultadoregla",
  "comportamiento": "restringido",
  "lookup": {
    "nombre": "sanic_reglaid",
    "displayname": "Regla",
    "descripcion": "Regla del catálogo que se evaluó.",
    "requerida": false,
    "auditoria": false
  }
}
...
Es una relación referencial con borrado restringido: el padre no se puede borrar mientras tenga hijos. Crea la columna lookup `sanic_reglaid` en `sanic_mppp_tbl_resultadoregla`.

Decisiones que este playbook toma y el diseño no traía:

| Decisión | Valor | Por qué |
|---|---|---|
| Nombre visible y descripción del lookup | «Regla» | El diccionario trae el nombre lógico; el texto de negocio lo fija el playbook (`02` DD-19) |

Viene del diseño, no lo decide este playbook:

- Comportamiento `restringido`: `02` §5 e inventario §4.
- Requerida = no: el `Req` de `sanic_reglaid` en `02` §3.3.
...
| 3 | Las dos tablas existen | `sanic_mppp_tbl_regla` (construida) y `sanic_mppp_tbl_resultadoregla` (construida) |
...
```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_regla_resultadoregla.md
```

## 5. Verificación

```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_regla_resultadoregla.md --solo-verificar
python3 herramientas/construir/muestra_relacion.py playbooks/relacion/sanic_mppp_regla_resultadoregla.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_resultadoregla.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET RelationshipDefinitions(SchemaName='sanic_mppp_regla_resultadoregla')` | 200 · `ReferencedEntity = sanic_mppp_tbl_regla` · `ReferencingEntity = sanic_mppp_tbl_resultadoregla` · `ReferencingAttribute = sanic_reglaid` · `IsCustomRelationship = true` · `IsManaged = false` |
...
| 3 | El lookup `sanic_reglaid` | tipo lookup · `Targets = [sanic_mppp_tbl_regla]` · requerida = false · auditoría = false · sin seguridad de columna · nombre visible «Regla» y su descripción en 1033, sin etiquetas en otro idioma |
| 4 | Pertenencia a la solución | la de la tabla hija: `sanic_mppp_tbl_resultadoregla` está una vez en la solución de la sección 1, con todos sus subcomponentes (una relación no es un componente propio) |
