# Playbook: relación · sanic_mppp_plan_autorizacionplan
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La relación está definida en `diseno/02-diccionario-datos.md` §2.4 y §5 y figura en `diseno/06-inventario-componentes.md` §4, renglón 4.4.
...
{
  "tipo": "relacion",
  "nombre": "sanic_mppp_plan_autorizacionplan",
  ...
  "lookup": {
    "nombre": "sanic_planid",
    "displayname": "Plan",
    "descripcion": "Plan sobre el que se autoriza al correo.",
    "requerida": true,
    "auditoria": true
  }
}
...
Es una relación referencial con borrado restringido: el padre no se puede borrar mientras tenga hijos. Crea la columna lookup `sanic_planid` en `sanic_mppp_tbl_autorizacionplan`.
...
- Requerida = sí: el `Req` de `sanic_planid` en `02` §2.4.
...
| 3 | Las dos tablas existen | `sanic_mppp_tbl_plan` (construida) y `sanic_mppp_tbl_autorizacionplan` (construida) |
...
```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_plan_autorizacionplan.md
```

## 5. Verificación

```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_plan_autorizacionplan.md --solo-verificar
python3 herramientas/construir/muestra_relacion.py playbooks/relacion/sanic_mppp_plan_autorizacionplan.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_autorizacionplan.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET RelationshipDefinitions(SchemaName='sanic_mppp_plan_autorizacionplan')` | 200 · `ReferencedEntity = sanic_mppp_tbl_plan` · `ReferencingEntity = sanic_mppp_tbl_autorizacionplan` · `ReferencingAttribute = sanic_planid` · `IsCustomRelationship = true` · `IsManaged = false` |
...
| 3 | El lookup `sanic_planid` | tipo lookup · `Targets = [sanic_mppp_tbl_plan]` · requerida = true · auditoría = true · sin seguridad de columna · nombre visible «Plan» y su descripción en 1033, sin etiquetas en otro idioma |
