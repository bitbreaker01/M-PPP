# Playbook: relación · sanic_mppp_systemuser_fila_digitadapor
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La relación está definida en `diseno/02-diccionario-datos.md` §3.2 y figura en `diseno/06-inventario-componentes.md` §4, renglón 4.10.
...
{
  "tipo": "relacion",
  "nombre": "sanic_mppp_systemuser_fila_digitadapor",
  "tabla_padre": "systemuser",
  "tabla_hija": "sanic_mppp_tbl_fila",
  "comportamiento": "restringido",
  "lookup": {
    "nombre": "sanic_digitadapor",
    "displayname": "Digitada por",
    "descripcion": "Quién marcó la fila como digitada. La escribe el plugin, nunca quien llama.",
    "requerida": false,
    "auditoria": false
  }
}
...
Es una relación referencial con borrado restringido: el padre no se puede borrar mientras tenga hijos. Crea la columna lookup `sanic_digitadapor` en `sanic_mppp_tbl_fila`.
...
- Requerida = no: el `Req` de `sanic_digitadapor` en `02` §3.2.
...
```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_systemuser_fila_digitadapor.md
```
...
|---|---|---|
| 1 | `GET RelationshipDefinitions(SchemaName='sanic_mppp_systemuser_fila_digitadapor')` | 200 · `ReferencedEntity = systemuser` · `ReferencingEntity = sanic_mppp_tbl_fila` · `ReferencingAttribute = sanic_digitadapor` · `IsCustomRelationship = true` · `IsManaged = false` |
...
| 3 | El lookup `sanic_digitadapor` | tipo lookup · `Targets = [systemuser]` · requerida = false · auditoría = false · sin seguridad de columna · nombre visible «Digitada por» y su descripción en 1033, sin etiquetas en otro idioma |
