# Playbook: relación · sanic_mppp_cliente_plan
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La relación está definida en `diseno/02-diccionario-datos.md` §2.2 y §5 y figura en `diseno/06-inventario-componentes.md` §4, renglón 4.1.
...
{
  "tipo": "relacion",
  "nombre": "sanic_mppp_cliente_plan",
  ...
  "lookup": {
    ...
    "displayname": "Cliente",
    "descripcion": "Empresa cliente a la que pertenece el plan. Un plan es de una sola empresa.",
    "requerida": true,
    "auditoria": true
  }
}
...
Es una relación referencial con borrado restringido: el padre no se puede borrar mientras tenga hijos. Crea la columna lookup `sanic_clienteid` en `sanic_mppp_tbl_plan`.
...
- Requerida = sí: el `Req` de `sanic_clienteid` en `02` §2.2.
...
| 3 | Las dos tablas existen | `sanic_mppp_tbl_cliente` (construida) y `sanic_mppp_tbl_plan` (construida) |
...
```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_cliente_plan.md
```

## 5. Verificación

```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_cliente_plan.md --solo-verificar
python3 herramientas/construir/muestra_relacion.py playbooks/relacion/sanic_mppp_cliente_plan.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_plan.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET RelationshipDefinitions(SchemaName='sanic_mppp_cliente_plan')` | 200 · `ReferencedEntity = sanic_mppp_tbl_cliente` · `ReferencingEntity = sanic_mppp_tbl_plan` · `ReferencingAttribute = sanic_clienteid` · `IsCustomRelationship = true` · `IsManaged = false` |
...
| 4 | Pertenencia a la solución | la de la tabla hija: `sanic_mppp_tbl_plan` está una vez en la solución de la sección 1, con todos sus subcomponentes (una relación no es un componente propio) |
