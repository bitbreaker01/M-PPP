# Playbook: relación · sanic_mppp_solicitud_resultadoregla
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La relación está definida en `diseno/02-diccionario-datos.md` §3.3 y §5 y figura en `diseno/06-inventario-componentes.md` §4, renglón 4.6.
...
{
  "tipo": "relacion",
  "nombre": "sanic_mppp_solicitud_resultadoregla",
  ...
  "lookup": {
    ...
    "descripcion": "Solicitud sobre la que se evaluó la regla.",
    "requerida": false,
    "auditoria": false
  }
}
...
Es una relación parental: asignar, compartir, reasignar y **borrar** el padre arrastra a los hijos. Crea la columna lookup `sanic_solicitudid` en `sanic_mppp_tbl_resultadoregla`.
...
- Requerida = no: el `Req` de `sanic_solicitudid` en `02` §3.3.
...
| 3 | Las dos tablas existen | `sanic_mppp_tbl_solicitud` (construida) y `sanic_mppp_tbl_resultadoregla` (construida) |
...
```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_solicitud_resultadoregla.md
```

## 5. Verificación

```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_solicitud_resultadoregla.md --solo-verificar
python3 herramientas/construir/muestra_relacion.py playbooks/relacion/sanic_mppp_solicitud_resultadoregla.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_resultadoregla.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET RelationshipDefinitions(SchemaName='sanic_mppp_solicitud_resultadoregla')` | 200 · `ReferencedEntity = sanic_mppp_tbl_solicitud` · `ReferencingEntity = sanic_mppp_tbl_resultadoregla` · `ReferencingAttribute = sanic_solicitudid` · `IsCustomRelationship = true` · `IsManaged = false` |
...
| 3 | El lookup `sanic_solicitudid` | tipo lookup · `Targets = [sanic_mppp_tbl_solicitud]` · requerida = false · auditoría = false · sin seguridad de columna · nombre visible «Solicitud» y su descripción en 1033, sin etiquetas en otro idioma |
