# Playbook: relación · sanic_mppp_plan_fila
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La relación está definida en `diseno/02-diccionario-datos.md` §3.2 y §5 y figura en `diseno/06-inventario-componentes.md` §4, renglón 4.8.
...
{
  "tipo": "relacion",
  "nombre": "sanic_mppp_plan_fila",
  "tabla_padre": "sanic_mppp_tbl_plan",
  "tabla_hija": "sanic_mppp_tbl_fila",
  "comportamiento": "restringido",
  "lookup": {
    "nombre": "sanic_planid",
    "displayname": "Plan",
    "descripcion": "Plan al que se refiere la fila. Vacío si el plan no existe. De acá sale la empresa de la fila para la vista de mis clientes.",
    "requerida": false,
    "auditoria": false
  }
}
...
Es una relación referencial con borrado restringido: el padre no se puede borrar mientras tenga hijos. Crea la columna lookup `sanic_planid` en `sanic_mppp_tbl_fila`.

Decisiones que este playbook toma y el diseño no traía:

| Decisión | Valor | Por qué |
|---|---|---|
| Nombre visible y descripción del lookup | «Plan» | El diccionario trae el nombre lógico; el texto de negocio lo fija el playbook (`02` DD-19) |

Viene del diseño, no lo decide este playbook:

- Comportamiento `restringido`: `02` §5 e inventario §4.
- Requerida = no: el `Req` de `sanic_planid` en `02` §3.2.
...
| 3 | Las dos tablas existen | `sanic_mppp_tbl_plan` (construida) y `sanic_mppp_tbl_fila` (construida) |
...
```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_plan_fila.md
```

## 5. Verificación

```
python3 herramientas/construir/relacion.py playbooks/relacion/sanic_mppp_plan_fila.md --solo-verificar
python3 herramientas/construir/muestra_relacion.py playbooks/relacion/sanic_mppp_plan_fila.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_fila.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET RelationshipDefinitions(SchemaName='sanic_mppp_plan_fila')` | 200 · `ReferencedEntity = sanic_mppp_tbl_plan` · `ReferencingEntity = sanic_mppp_tbl_fila` · `ReferencingAttribute = sanic_planid` · `IsCustomRelationship = true` · `IsManaged = false` |
...
| 3 | El lookup `sanic_planid` | tipo lookup · `Targets = [sanic_mppp_tbl_plan]` · requerida = false · auditoría = false · sin seguridad de columna · nombre visible «Plan» y su descripción en 1033, sin etiquetas en otro idioma |
