# Playbook: clave alternativa · sanic_mppp_key_cliente_cifcom
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La clave está definida en `diseno/02-diccionario-datos.md` §2.1 y figura en `diseno/06-inventario-componentes.md` §5, renglón 5.2.
...
{
  "tipo": "clave",
  "nombre": "sanic_mppp_key_cliente_cifcom",
  "displayname": "KEY - MPPP - Cliente - CIF COM",
...
Una clave alternativa sobre `sanic_mppp_tbl_cliente`: la plataforma no admite dos registros con la misma combinación de `sanic_cifcom`.
...
|---|---|---|
| Nombre visible de la clave | «KEY - MPPP - Cliente - CIF COM» | El inventario trae el nombre lógico; el visible sigue el patrón `KEY - MPPP - Tabla - Campos` (`01` §2) |

Viene del diseño, no lo decide este playbook:

- Columnas de la clave: `02` §2.1 e inventario §5, renglón 5.2.
- Un solo cliente por CIF COM.
...
| 4 | Cada columna de la clave existe, es de un tipo que admite clave y no tiene seguridad de columna | `sanic_cifcom` |
...
```
python3 herramientas/construir/clave.py playbooks/clave/sanic_mppp_key_cliente_cifcom.md
```

## 5. Verificación

```
python3 herramientas/construir/clave.py playbooks/clave/sanic_mppp_key_cliente_cifcom.md --solo-verificar
python3 herramientas/construir/muestra_clave.py playbooks/clave/sanic_mppp_key_cliente_cifcom.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_cliente.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET EntityDefinitions(LogicalName='sanic_mppp_tbl_cliente')/Keys(LogicalName='sanic_mppp_key_cliente_cifcom')` | 200 · `IsManaged = false` · nombre visible «KEY - MPPP - Cliente - CIF COM» en 1033, sin etiquetas en otro idioma |
| 2 | Columnas de la clave (`KeyAttributes`, sin importar el orden) | `sanic_cifcom` |
