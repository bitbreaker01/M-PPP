# Playbook: clave alternativa · sanic_mppp_key_fila_solicitud_numerofila
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La clave está definida en `diseno/02-diccionario-datos.md` §3.2 y figura en `diseno/06-inventario-componentes.md` §5, renglón 5.9.
...
{
  "tipo": "clave",
  "nombre": "sanic_mppp_key_fila_solicitud_numerofila",
  "displayname": "KEY - MPPP - Fila - Solicitud y numero de fila",
...
Una clave alternativa sobre `sanic_mppp_tbl_fila`: la plataforma no admite dos registros con la misma combinación de `sanic_solicitudid` · `sanic_numerofila`.
...
|---|---|---|
| Nombre visible de la clave | «KEY - MPPP - Fila - Solicitud y numero de fila» | El inventario trae el nombre lógico; el visible sigue el patrón `KEY - MPPP - Tabla - Campos` (`01` §2) |

Viene del diseño, no lo decide este playbook:

- Columnas de la clave: `02` §3.2 e inventario §5, renglón 5.9.
- Un número de fila no se repite dentro de una solicitud.
...
| 3 | La tabla existe | `sanic_mppp_tbl_fila` (construida) |
| 4 | Cada columna de la clave existe, es de un tipo que admite clave y no tiene seguridad de columna | `sanic_solicitudid` · `sanic_numerofila` |
...
```
python3 herramientas/construir/clave.py playbooks/clave/sanic_mppp_key_fila_solicitud_numerofila.md
```

## 5. Verificación

```
python3 herramientas/construir/clave.py playbooks/clave/sanic_mppp_key_fila_solicitud_numerofila.md --solo-verificar
python3 herramientas/construir/muestra_clave.py playbooks/clave/sanic_mppp_key_fila_solicitud_numerofila.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_fila.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET EntityDefinitions(LogicalName='sanic_mppp_tbl_fila')/Keys(LogicalName='sanic_mppp_key_fila_solicitud_numerofila')` | 200 · `IsManaged = false` · nombre visible «KEY - MPPP - Fila - Solicitud y numero de fila» en 1033, sin etiquetas en otro idioma |
| 2 | Columnas de la clave (`KeyAttributes`, sin importar el orden) | `sanic_solicitudid` · `sanic_numerofila` |
| 3 | Índice (`EntityKeyIndexStatus`) | `Active` |
| 4 | Pertenencia a la solución | la de la tabla: `sanic_mppp_tbl_fila` está una vez en la solución de la sección 1, con todos sus subcomponentes (una clave no es un componente propio) |
...
`DELETE EntityDefinitions(LogicalName='sanic_mppp_tbl_fila')/Keys(<MetadataId>)`. No borra datos ni columnas: solo deja de exigir la unicidad. **Irreversible desde que se crea**: el nombre de la clave. Las columnas de una clave no se editan: para cambiarlas se borra la clave y se crea otra. Mientras la clave exista, sus columnas no se pueden borrar.

## 8. Fuera de alcance

- La relación que crea el lookup de esta clave: inventario 4.5, ya construida.
