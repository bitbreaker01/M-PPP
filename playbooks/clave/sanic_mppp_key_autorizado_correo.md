...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La clave está definida en `diseno/02-diccionario-datos.md` §2.3 y figura en `diseno/06-inventario-componentes.md` §5, renglón 5.4.
...
- Columnas de la clave: `02` §2.3 e inventario §5, renglón 5.4.
- Un correo figura una sola vez por cliente. La clave no distingue mayúsculas (spike C-05).
...
| 3 | La tabla existe | `sanic_mppp_tbl_autorizado` (construida) |
| 4 | Cada columna de la clave existe, es de un tipo que admite clave y no tiene seguridad de columna | `sanic_clienteid` · `sanic_nombre` |
...
- La relación que crea el lookup de esta clave: inventario 4.2, ya construida.
