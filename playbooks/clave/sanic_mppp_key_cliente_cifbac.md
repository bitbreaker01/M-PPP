# Playbook: clave alternativa · sanic_mppp_key_cliente_cifbac
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. La clave está definida en `diseno/02-diccionario-datos.md` §2.1 y figura en `diseno/06-inventario-componentes.md` §5, renglón 5.1.
...
{
  "tipo": "clave",
  "nombre": "sanic_mppp_key_cliente_cifbac",
  "displayname": "KEY - MPPP - Cliente - CIF BAC",
...
Una clave alternativa sobre `sanic_mppp_tbl_cliente`: la plataforma no admite dos registros con la misma combinación de `sanic_cifbac`.
...
|---|---|---|
| Nombre visible de la clave | «KEY - MPPP - Cliente - CIF BAC» | El inventario trae el nombre lógico; el visible sigue el patrón `KEY - MPPP - Tabla - Campos` (`01` §2) |

Viene del diseño, no lo decide este playbook:

- Columnas de la clave: `02` §2.1 e inventario §5, renglón 5.1.
- Un solo cliente por CIF BAC (9 dígitos).
...
| 3 | La tabla existe | `sanic_mppp_tbl_cliente` (construida) |
| 4 | Cada columna de la clave existe, es de un tipo que admite clave y no tiene seguridad de columna | `sanic_cifbac` |
...
```
python3 herramientas/construir/clave.py playbooks/clave/sanic_mppp_key_cliente_cifbac.md
```

## 5. Verificación

```
python3 herramientas/construir/clave.py playbooks/clave/sanic_mppp_key_cliente_cifbac.md --solo-verificar
python3 herramientas/construir/muestra_clave.py playbooks/clave/sanic_mppp_key_cliente_cifbac.md --guardar
python3 herramientas/construir/tabla.py playbooks/tabla/sanic_mppp_tbl_cliente.md --solo-verificar
```

| # | Comprobación | Esperado |
|---|---|---|
| 1 | `GET EntityDefinitions(LogicalName='sanic_mppp_tbl_cliente')/Keys(LogicalName='sanic_mppp_key_cliente_cifbac')` | 200 · `IsManaged = false` · nombre visible «KEY - MPPP - Cliente - CIF BAC» en 1033, sin etiquetas en otro idioma |
| 2 | Columnas de la clave (`KeyAttributes`, sin importar el orden) | `sanic_cifbac` |
| 3 | Índice (`EntityKeyIndexStatus`) | `Active` |
| 4 | Pertenencia a la solución | la de la tabla: `sanic_mppp_tbl_cliente` está una vez en la solución de la sección 1, con todos sus subcomponentes (una clave no es un componente propio) |
...
`DELETE EntityDefinitions(LogicalName='sanic_mppp_tbl_cliente')/Keys(<MetadataId>)`. No borra datos ni columnas: solo deja de exigir la unicidad. **Irreversible desde que se crea**: el nombre de la clave. Las columnas de una clave no se editan: para cambiarlas se borra la clave y se crea otra. Mientras la clave exista, sus columnas no se pueden borrar.
