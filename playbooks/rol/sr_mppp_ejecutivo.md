# Playbook: rol · SR - MPPP - Ejecutivo
...
De dónde sale cada dato: `diseno/01-convenciones.md` §0 y §7, verificados contra Dev el 2026-09-20. El rol está definido en `diseno/04-matriz-privilegios.md` §2 y figura en `diseno/06-inventario-componentes.md` §6, renglón 6.2.
...
{
  "tipo": "rol",
  "nombre": "SR - MPPP - Ejecutivo",
  "descripcion": "Ejecutivo de cuenta: consulta catálogos y solicitudes, y cambia el estado de las filas que digita en AS400.",
  "base": "App Opener",
  "tablas": {
...
Un security role en la unidad de negocio raíz, con los privilegios del rol `App Opener` (los que tenga el entorno en el momento de construir) más los 13 de este playbook. Alcances: `usuario` = Basic (solo lo propio) · `unidad` = Local · `unidad_e_hijas` = Deep · `organizacion` = Global.

Decisiones que este playbook toma y el diseño no traía:

| Decisión | Valor | Por qué |
|---|---|---|
| Descripción del rol | «Ejecutivo de cuenta: consulta catálogos y solicitudes, y cambia el estado de las filas que digita en AS400.» | La matriz trae el nombre y los privilegios; el texto lo fija el playbook |

Viene del diseño, no lo decide este playbook:

- Privilegios: `04` §2, columna Ejecutivo.
- Nadie borra (`04` §2).
- Tiene `escribir` sobre Fila y Solicitud, pero qué columnas puede tocar lo limita la lista blanca del plugin (`04` §1), no este rol.
- Ejecutivo y Supervisor son mutuamente excluyentes por usuario (D-25): lo controla el plugin.
...
```
python3 herramientas/construir/rol.py playbooks/rol/sr_mppp_ejecutivo.md
```

## 5. Verificación

```
python3 herramientas/construir/rol.py playbooks/rol/sr_mppp_ejecutivo.md --solo-verificar
python3 herramientas/construir/muestra_rol.py playbooks/rol/sr_mppp_ejecutivo.md --guardar
```
...
| `sanic_mppp_tbl_solicitud` | `prvReadsanic_mppp_tbl_solicitud` = Global · `prvWritesanic_mppp_tbl_solicitud` = Global · `prvAppendTosanic_mppp_tbl_solicitud` = Global |
